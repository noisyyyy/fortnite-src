...
#define code_rw CTL_CODE(FILE_DEVICE_UNKNOWN, 0x543, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define code_ba CTL_CODE(FILE_DEVICE_UNKNOWN, 0x421, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
//#define code_get_guarded_region CTL_CODE(FILE_DEVICE_UNKNOWN, 0x73, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)
#define code_security 0x901641
typedef struct _rw {
	INT32 security;
	INT32 process_id;
	ULONGLONG address;
...
namespace mem {
	...
	bool find_driver() {
		SPOOF_FUNC;
		driver_handle = CreateFileW(skCrypt(L"\\\\.\\ghettodielit"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
